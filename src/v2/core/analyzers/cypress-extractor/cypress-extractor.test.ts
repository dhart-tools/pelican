...
describe("CypressExtractorAnalyzer", () => {
  ...
  test("extract(): should process a complete login flow", async () => {
    ...
    expect(result.interceptedAPIs.some(i => i.method === "POST" && i.urlPattern === "/api/login")).toBe(true);
    expect(result.containsText).toContain("Welcome, testuser");
    expect(result.urlAssertions.some(a => a.operator === "include" && a.expectedValue === "/dashboard")).toBe(true);
  });
  ...
  test("extractSelectors(): should handle various selector types", async () => {
    ...
    expect(result.selectors.some(s => s.type === 'testid' && s.value === 'test-id')).toBe(true);
    expect(result.selectors.some(s => s.type === 'data-cy' && s.value === 'cy-id')).toBe(true);
    expect(result.selectors.some(s => s.type === 'id' && s.value === 'my-id')).toBe(true);
    expect(result.selectors.some(s => s.type === 'class' && s.value === 'my-class')).toBe(true);
    expect(result.selectors.some(s => s.type === 'complex' && s.value === 'div > p')).toBe(true);
  });
  ...
  test("extractIntercept(): should handle different intercept signatures", async () => {
    ...
    expect(result.interceptedAPIs.some(i => i.method === 'GET' && i.urlPattern === '/api/simple')).toBe(true);
    expect(result.interceptedAPIs.some(i => i.method === 'POST' && i.urlPattern === '/api/complex')).toBe(true);
  });
