...
describe("SelectorMatchScorer", () => {
  ...
  beforeEach(() => {
    ...
    mockContext = {
      testFile: {
        cypress: {
          selectors: [{ type: ESelectorAttr.TEST_ID, value: "login-submit", raw: '[data-testid="login-submit"]' }]
        }
      } as any,
      changedFile: {
  ...
  test("evaluate(): should detect matching data-testid selectors", () => {
    ...
    expect(signals[0].weight).toBe(0.80);
  ...
  test("evaluate(): should detect matching data-cy selectors", () => {
    mockContext.testFile!.cypress!.selectors = [{ type: ESelectorAttr.DATA_CY, value: "submit-button", raw: '[data-cy="submit-button"]' }];
